...
%             %pause
         ...
         %% estimate theta
                ...
                % using fit
                 ...
                 yid_fim_baseline;
         [xData, yData] = prepareCurveData( xid_fim_baseline, log10(yid_fim_baseline) );
...
%              
%             [y,FIM]=SSfim_cost_modified(no_ini_samples_fim,n,0,xid_fim_baseline,theta_est(n,:),[],sigma_y,0);
%             CRB(n,:)=diag(inv(FIM));
...
%             
%             [y,FIM_true]=SSfim_cost_modified(no_ini_samples_fim,n,0,xid,theta_true,[],sigma_y,0);
%             CRB_true(n,:)=diag(inv(FIM_true));
% 
%             var_CRB(n,1)=mean((CRB_true((no_ini_samples_fim:n),1)-mean(CRB_true((no_ini_samples_fim:n),1))).^2);
%             var_CRB(n,2)=mean((CRB_true((no_ini_samples_fim:n),2)-mean(CRB_true((no_ini_samples_fim:n),2))).^2);
%             var_CRB(n,3)=mean((CRB_true((no_ini_samples_fim:n),3)-mean(CRB_true((no_ini_samples_fim:n),3))).^2);
%             var_CRB(n,4)=mean((CRB_true((no_ini_samples_fim:n),4)-mean(CRB_true((no_ini_samples_fim:n),4))).^2);
%leila
             number_of_push=evalin('base','number_of_push');
%              
%              n=(number_of_push+1)/2+4;
%              nxeq0=evalin('base','nxeq0');
%              m=length(xid_fim_baseline);
%              
%              m
%              n
%              
...
%           
         ...
         pdpnts=plot(xid_fim_baseline(handles.nxeq0:end-1),yid_fim_baseline(handles.nxeq0:end-1),...
            'LineStyle','none',...
            'LineWidth',1,...
            'Marker','o',...
            'MarkerSize',8,...
...
%            
        %pause(dtime);
           ...
           pnewpoint=plot(xid_fim_baseline(end),(yid_new(end)),...
            'LineStyle','none',...
            'LineWidth',1,...
            'Marker','o',...
            'MarkerSize',8,...
...
%              poldfit=plot(xval,log(yest_val(n,1:end)),...
%             'LineStyle','-',...
%             'LineWidth',1,...
%             'Color','b');
         hold on
        ...
        hold on
        axes(handles.axes1);
            poldfit=plot(xval,1e6*10.^(yest_val(n-1,1:end)),...
            'LineStyle','--',...
            'LineWidth',1,...
            'Color','b');
        
        AX=legend('Baseline sample','Previous TMS sample','New TMS sample','Previous estimate of IO cruve','New estimate of IO curve','location','northwest');
...
%             while length(iter_rtheta_converged)~=0
